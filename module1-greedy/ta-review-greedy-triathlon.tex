...
If we let them start in the order $(c_1, c_2)$, $c_1$'s completion time is $t_{c_1} = w_1 + b_1 + r_1 = 21$, and $c_2$'s completion time is $t_{c_2} = w_1 + w_2 + b_2 + r_2 = 26$, as he has to wait for the first contestant to finish his swim. Therefore, the overall completion time $t_c = max(t_{c_1}, t_{c_2}) = 26$.\\
...
Therefore, optimal order is $(c_1, c_2)$ with $t_c = 26$.
